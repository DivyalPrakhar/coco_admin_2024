...
export const updateMemberAction = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		...
		if (ok) return { data, extraData: payload.batch };
		else return thunkAPI.rejectWithValue(problem);
	}
);
...
export const deleteAlumniEducationAction = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		...
		if (ok) return { data, extraData: payload };
		else return thunkAPI.rejectWithValue(problem);
	}
);
...
export const assignStudPkgAction = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		...
		const { ok, problem, data } = response;
		console.log(
			"payload from action creater",
			payload,
			"response from action creator studens",
			response
		);
...
export const updateStudentPackageAction = createAsyncThunk(
	...
	async (payload, thunkAPI) => {
		const response = await apis.updateStudentPackageDetails(
			payload
		);
		const { ok, problem, data } = response;
...
const studentSlice = createSlice({
	...
	reducers: {
		...
		resetExperienceStatusAction: (state) => {
			...
			state.deleteStudentExperienceStatus =
				STATUS.NOT_STARTED;
		},
	...
	extraReducers: {
		...
		[updateStudentPackageAction.fulfilled]: (state, action) => {
			...
			state.currentStudent.packages =
				state.currentStudent.packages.map((pkg) => ({
					...pkg,
					validity: _.find(
		...
		[removeStudPkgeAction.fulfilled]: (state, action) => {
			...
			_.remove(
				...
				(p) =>
					p.package._id ===
					action.payload.extraData.packageId
			);
		...
		[addEducationAction.fulfilled]: (state, action) => {
			...
			state.currentStudent = Object.assign(
				{},
				state.currentStudent,
				{
					educations: state.currentStudent
						.educations
						? _.concat(
								state
									.currentStudent
		...
		[editEducationAction.fulfilled]: (state, action) => {
			let findEducation = state.currentStudent.educations
				? _.findIndex(
						state.currentStudent.educations,
						(s) => s.id == action.payload.id
				  )
				: -1;
			if (findEducation != -1) {
				state.currentStudent.educations[findEducation] =
					action.payload;
			}
		...
		[deleteAlumniEducationAction.fulfilled]: (state, action) => {
			...
			state.currentStudent.educations = _.filter(
				state.currentStudent.educations,
				(g) => g.id != action.payload.extraData.id
			);
		...
		[assignGroupsAction.fulfilled]: (state, action) => {
			...
			action.payload.map((grp) =>
				state.currentStudent.groups.push(grp)
			);
		},
		...
		[updateStudentAction.fulfilled]: (state, action) => {
			...
			state.currentStudent.user = Object.assign(
		...
		[addExperienceAction.fulfilled]: (state, action) => {
			...
			state.currentStudent = Object.assign(
				...
				state.currentStudent,
				{
					experiences: state.currentStudent
						.experiences
						? _.concat(
								state
									.currentStudent
		...
		[editExperienceAction.fulfilled]: (state, action) => {
			let findExperience = state.currentStudent.experiences
				? _.findIndex(
						state.currentStudent
							.experiences,
						(s) => s.id == action.payload.id
				...
				: -1;
			if (findExperience != -1) {
				state.currentStudent.experiences[
					findExperience
				] = action.payload;
			}
		...
		[deleteAlumniExperienceAction.fulfilled]: (state, action) => {
			SuccessMessage("Profile Updated");
			state.currentStudent.experiences = _.filter(
				state.currentStudent.experiences,
				(g) => g.id != action.payload.extraData.id
			);
		...
		[removeStudCourseAction.fulfilled]: (state, action) => {
			...
			_.remove(
				state.currentStudent.courses,
				(d) =>
					d.course._id ==
					action.payload.extraData.courseId
			);
		...
		[assignBatchAction.fulfilled]: (state, action) => {
			...
			// state.currentStudent= action.payload
			state.studentAssignedBatch = concat(
				state.studentAssignedBatch,
				action.payload
			);

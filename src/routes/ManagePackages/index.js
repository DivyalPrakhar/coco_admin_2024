...
export const ManagePackages = () => {
  ...
  const { packages, configData, products, course, testsList } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    if (configData.defaultDataStatus == STATUS.SUCCESS) {
      ...
      if (defaultData) setDefaultSyllabus({ exams: defaultData.exams });
    }
  ...
  const actionsList = [
    ...
    { title: "Students", id: 4, callback: pkgStudents, icon: <UserOutlined /> },
    {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (d) => (
        <Tag
          color={d == 1 ? "green" : d == 0 ? "red" : d == 2 ? "orange" : "red"}
        >
          {publishStatuses[d || 0]}
        </Tag>
      ),
    ...
    { title: <b>Priority</b>, width: 100, dataIndex: "priority", key: 1 },
    {
      ...
      render: (ids) => {
        ...
        return (
          <div>
            {exams.length
              ? exams.map((exam) => <Tag>{exam.name.en}</Tag>)
              : null}
          </div>
        );
      },
      ...
      render: (d) => {
        ...
        return (
          <>
            ...
            {books.length ? (
              <div>
                <b>Books:</b> {books.length}
              </div>
            ) : null}
            {drives.length ? (
              <div>
                <b>Drives:</b> {drives.length}
              </div>
            ) : null}
            {magazines.length ? (
              <div>
      ...
      render: (d) => {
        return (
          <Popover
            ...
            onVisibleChange={() => toggleAction(d._id)}
            content={
              <List
                dataSource={actionsList}
                renderItem={(item) => (
                  <List.Item
                    onClick={() => item.callback(d._id)}
  ...
  let dataSource = useMemo(() => {
    return packages.packagesList?.length
      ? _.orderBy(packages.packagesList, ["createdAt"], ["desc"]).map(
          (pkg) => ({
            ...
            price: {
              ...
              isSubscription: pkg.priceMode === "sub",
            },
            actions: pkg,
            serial: pkg.serial,
  ...
  useEffect(() => {
    ...
    if (finalFilters)
      data = {
        ...finalFilters,
        exams: finalFilters.exams ? [finalFilters.exams] : [],
      };
  ...
  const filterPackages = () => {
    ...
    if (published || published === 0) setQuery("published", published);
  ...
  return (
    <div>
      ...
      <Card
        loading={
          configData.defaultDataStatus == STATUS.FETCHING ||
          products.allProductStatus == STATUS.FETCHING ||
        ...
        course.getCoursesStatus == STATUS.SUCCESS ? (
          <div>
            ...
            <Table
              bordered
              scroll={{ x: 1800 }}
              loading={packages.getPackagesStatus === STATUS.FETCHING}
              dataSource={dataSource}
              columns={columns}
              key={dataSource?.length}
              pagination={{
                position: ["bottomCenter"],
                current: parseInt(params?.pageNumber) || 1,
              }}
              onChange={changePage}
            />
          </div>

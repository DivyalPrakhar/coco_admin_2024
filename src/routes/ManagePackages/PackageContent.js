...
import {
	addAssignmentToPkgAction,
	removePkgAssignmentAction,
	schedulePackageTestAction,
	updatePackageContentAction,
	updatePkgAssignmentAction,
...
export const PackageContent = ({ currentPackage, contentTypes }) => {
	...
	useEffect(() => {
		if (packageReducer.updatePkgContentStatus === STATUS.SUCCESS) _setRequiredContents();
	...
	const checkContentType = (type) => {
		return (
			_.findIndex(requiredContents, (d) => d == type) != -1 ||
			currentPackage[_.lowerCase(type) + "s"]?.length
		);
	};
	...
	return (
		<Card
			loading={
				...
				product.allProductStatus == STATUS.FETCHING ||
				assignments.getAssignmentsStatus == STATUS.FETCHING
			}
			style={{ border: 0, minHeight: "500px" }}
			bodyStyle={{ padding: 0 }}>
			{course.getCoursesStatus == STATUS.SUCCESS &&
			product.allProductStatus == STATUS.SUCCESS &&
			assignments.getAssignmentsStatus == STATUS.SUCCESS ? (
				<div>
...
const MagazineCard = ({ product, assignData, currentPackage, loading }) => {
	...
	const _assignData = () => {
		...
		assignData({
			value: _.compact([...currentMagazineIds, ...selectedMagazines]),
			path: "magazines",
		});
	};
	...
	const removeMagazine = (id) => {
		ConfirmAlert(
			() => assignData({ value: id, path: "magazines", remove: true }),
			"Are you Sure?"
		);
	};
	const magazines = product.productsData?.MAGAZINE || [];
	const magazinesList = magazines.length
		? currentPackage.magazines
			? _.differenceBy(magazines, currentPackage.magazines, "_id")
			: magazines
		: [];
	...
	return (
		<Card bodyStyle={{ padding: "10px" }} loading={loading}>
			<div style={{ background: "#E8F6F3", padding: "5px 20px" }}>
				<Text style={{ fontWeight: "bold", color: "#3498DB", fontSize: "18px" }}>
					Magazines
				</Text>
			</div>
			...
			<Table bordered dataSource={assignedMagazines} pagination={false}>
				<Table.Column
					title="Name"
					dataIndex="name"
					key="name"
					render={(d) => <Text>{d?.en}</Text>}></Table.Column>
				<Table.Column
					title="Description"
				...
				<Table.Column
					...
					align="center"
					render={(data) => (
						<Button danger onClick={() => removeMagazine(data._id, "magazines")}>
...
const DriveCard = ({ product, assignData, currentPackage, loading }) => {
	...
	const _assignData = () => {
		...
		assignData({
			value: _.compact([...currentDriveIds, ...selectedDrives]),
			path: "drives",
		});
	};
	...
	const removeDrive = (id) => {
		ConfirmAlert(
			() => assignData({ value: id, path: "drives", remove: true }),
			"Are you Sure?"
		);
	};

	const drives = product?.productsData?.DRIVE || [];
	const drivesList = drives.length
		? currentPackage.drives
			? _.differenceBy(drives, currentPackage.drives, "_id")
			: drives
		: [];
	...
	return (
		<Card
			...
			<Table bordered dataSource={assignedDrives} pagination={false}>
				<Table.Column
					title="Name"
					dataIndex="name"
					key="name"
				...
				<Table.Column
					...
					align="center"
					render={(data) => (
						<Button danger onClick={() => removeDrive(data._id, "drives")}>
							<DeleteOutlined style={{ fontSize: "14px" }} />
						</Button>
					)}></Table.Column>
			</Table>
...
const BookCard = ({ product, assignData, currentPackage, loading }) => {
	...
	const _assignData = () => {
		...
		assignData({
			value: _.compact([...currentBookIds, ...selectedBooks]),
			path: "books",
		});
	};
	...
	const removeBook = (id) => {
		ConfirmAlert(
			() => assignData({ value: id, path: "books", remove: true }),
			"Are you Sure?"
		);
	};
	...
	const booksList = books.length
		? currentPackage.books
			? _.differenceBy(books, currentPackage.books, "_id")
			: books
		: [];
	...
	return (
		<Card
			...
			<Table bordered dataSource={assignedBooks} pagination={false}>
				<Table.Column
					title="Name"
					dataIndex="name"
					key="name"
				...
				<Table.Column
					...
					align="center"
					render={(data) => (
						<Button danger onClick={() => removeBook(data._id, "books")}>
							<DeleteOutlined style={{ fontSize: "14px" }} />
						</Button>
					)}></Table.Column>
			</Table>
...
const CourseCard = ({ course, assignData, currentPackage, loading }) => {
	...
	const _assignData = () => {
		...
		assignData({
			value: _.compact([...currentCourseIds, ...selectedCourses]),
			path: "courses",
		});
	};
	...
	const removeCourse = (id, type) => {
		ConfirmAlert(
			() => assignData({ value: id, path: type, remove: true }),
			"Are you Sure?"
		);
	};
		...
		<Card
			...
			<Form.Item label="Select Course">
				<Select
					...
					{courseList.length
						? _.map(
								_.filter(courseList, (s) => s.isActive),
								(crs) => <Select.Option key={crs._id}>{crs.name}</Select.Option>
						  )
						: null}
				</Select>
				<Button
			...
			<Table bordered dataSource={assignedCourses} pagination={false}>
				<Table.Column
					title="Name"
					dataIndex="name"
					key="name"
					render={(d) => <Text>{d}</Text>}></Table.Column>
				<Table.Column
					title="Description"
					dataIndex="description"
					key="description"
				...
				<Table.Column
					...
					align="center"
					render={(data) => (
						<Button danger onClick={() => removeCourse(data._id, "courses")}>
							<DeleteOutlined style={{ fontSize: "14px" }} />
						</Button>
					)}></Table.Column>
			</Table>
...
const AssignmentCard = ({ assignments, currentPackage, loading }) => {
	...
	const { addAssignmentStatus, updateAssignmentStatus, removeAssignmentStatus } =
		useSelector((state) => ({
	...
	const _updateAssignedAssignments = () => {
		let data = currentPackage.assignments?.length
			? _.intersectionBy(
					assignments.map((d) => ({ ...d, assignmentId: d._id })),
					currentPackage.assignments,
					"assignmentId"
			  )
			: [];
		data = data.map((d) => {
			...
			return {
				...d,
				maximumMarks: assnmnt?.maximumMarks,
				startDate: assnmnt?.startDate,
				endDate: assnmnt?.endDate,
			};
		});
	...
	useEffect(() => {
		...
		if (
			...
			removeAssignmentStatus == STATUS.SUCCESS
		) {
			_updateAssignedAssignments();
		}
	...
	const _assignData = (assgnmnts) => {
		...
		data = {
			...
			assignments: data.map((a) => ({ assignmentId: a._id })),
		};
	...
	const updateAssignments = (id) => {
		...
		data = {
			packageId: currentPackage._id,
			assignments: assignedAssignments.map((d) => ({
				assignmentId: d._id,
				maximumMarks: parseInt(d.maximumMarks),
	...
	return (
		<Card
			...
			<Divider style={{ margin: "10px" }} />
			<Button type="primary" ghost onClick={openSelectAssignmentModal}>
				Select Assignment
			</Button>
			<br />
			<br />
				...
				<Table.Column
					...
					render={(d, row) => (
						<Input
							...
							min="0"
							value={d}
							onChange={(e) => changeMaxMarks(e, row._id)}
					...
					render={(d, row) => (
						<DatePicker.RangePicker
							...
							value={[
								...
								row.endDate ? moment(row.endDate) : null,
							]}
						...
						<Space>
							<Tooltip title="remove" placement="right">
								<Button
									danger
									icon={<DeleteOutlined />}
									onClick={() => removeAssignment(row._id)}></Button>
							</Tooltip>
						</Space>
					)}></Table.Column>
			</Table>
			...
			{selectAssignmentModal ? (
				<SelectAssignmentModal
					alreadySelected={assignedAssignments}
					loading={addAssignmentStatus == STATUS.FETCHING}
					visible={selectAssignmentModal}
					onfinish={_assignData}
					closeModal={openSelectAssignmentModal}
				/>
			) : null}
		</Card>
	);
...
const TestCard = ({ assignData, currentPackage, loading, scheduleTest }) => {
	...
	const removeTest = (data, type) => {
		ConfirmAlert(
			() => assignData({ value: data, path: type, remove: true }),
			"Are you Sure?"
		);
	};
	...
	// Reverse the order of the tests array so that when the table is rendered it shows the latest selected test first.
	currentPackage?.tests.reverse();
	...
	return (
		<Card
			...
			<Divider style={{ margin: "10px" }} />
			<Button type="primary" ghost onClick={() => selectTestModalChange(true)}>
				Select Test
			</Button>
			<br />
			<br />
			<Table bordered dataSource={assignedTests}>
				<Table.Column
					title="Ref Id"
				...
				<Table.Column
					title="Questions"
					key="queCont"
					render={(d) => _.size(_.flatMap(d.sections, "questions"))}
				/>
				...
				<Table.Column
					...
					key="date"
					render={(data) => {
						let startDate = data?.testPackageDetails?.startDate
							? moment(data?.testPackageDetails?.startDate)
							: null;
						let endDate = data?.testPackageDetails?.endDate
							? moment(data?.testPackageDetails?.endDate)
							: null;
						return (
							<DatePicker.RangePicker
								key={startDate}
								showTime
								allowClear={false}
								onChange={(e) => changeDateRange(e, data?.testPackageDetails?._id)}
				...
				<Table.Column
					...
					render={(data) => (
						<Button
							danger
							onClick={() => removeTest(data.testPackageDetails._id, "tests")}>
							<DeleteOutlined style={{ fontSize: "14px" }} />
						</Button>
					)}
				/>
			...
			{selectTestModal ? (
				<SelectTestForPackageModal
					preSelectedTests={_.map(currentPackage?.tests, (s) => s.test)}
					visibility={selectTestModal}
					closeModal={() => selectTestModalChange()}
					submitTestData={(data) => _assignData(data, "tests")}
				/>
			) : null}
		</Card>
	);

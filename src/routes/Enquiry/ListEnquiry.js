...
import { Button, Card, Col, DatePicker, Form, Input, Modal, Radio, Row, Select, Space, Table, Tag, Tooltip } from "antd";
...
import { getEnquiryAction, updateEnquiryAction } from "../../redux/reducers/offlineEnquiry/Enquiry";
...
export default function ListEnquiry() {
    ...
    const [toggleEditEnquiryModal, setToggleEditEnquiryModal] = useState(null)
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: 'course',
            key: '_id',
            render: d => find(allOfflineCourses, course => course._id === d)?.name || '-'
        },
        // {
        //     title: 'Remark',
        //     dataIndex: 'remark',
        ...
        },
        {
            ...
            dataIndex: 'status',
            key: '_id',
            render: d => <div>{d === "OPEN" ? <Tag color="#87d068">OPEN</Tag> : d === "CHEQUE_PAYMENT" ? <Tag color="#87d068">CHEQUE PAYMENT</Tag> : d === "CLOSED" ? <Tag color="#f50">CLOSED</Tag> : d === "ADMITTED" ? <Tag color="#1677ff">ADMITTED</Tag> : null}</div>
        },
        {
            title: 'Action',
            dataIndex: '',
            ...
            render: d => {
                return (
                    <div>
                        <Space>
                            <Tooltip title='Edit'>
                                <Button icon={<EditFilled style={{ color: '#1890ff' }} />} onClick={() => setToggleEditEnquiryModal(d)}></Button>
                            </Tooltip>
                            {/* <Tooltip title='Delete'>
                                <Button icon={<DeleteTwoTone twoToneColor='#eb2f96' />}></Button>
                            </Tooltip> */}
                            <Tooltip title='View Remarks'>
                                <Button onClick={() => setToggleRemarkDrawer(d)}>Remarks</Button>
                            </Tooltip>
                        </Space>
                    </div>
                )
    ...
    return (
        <div>
            <CommonPageHeader
                title="List Enquiry"
                extra={
                    <Button shape='round' icon={<PlusOutlined />} onClick={() => history.push('/enquiry-form')} size='large'>Add New Enquiry</Button>
                }
            />
            <br />
            <Card>
                        ...
                        <Row align="bottom">
                            <Col style={{ width: "70%" }}>
                                <Space>
                                    ...
                                    <CustomInput label="Contact" name='contact' width='200px' placeholder='Contact' labelCol={{ span: 6 }} />
                                    <Form.Item label="Course" name='course' labelCol={{ offset: 1, span: 7 }}>
                                ...
                                <Space>
                                    ...
                                    <Form.Item label="Start Date" name='startDate' labelCol={{ offset: 0, span: 6 }}>
                                        <DatePicker style={{ width: 200 }} name placeholder='Start Date' />
                                    </Form.Item>
                                    <Form.Item label="End Date" name='endDate' labelCol={{ offset: 0, span: 6 }}>
                                        <DatePicker style={{ width: 200 }} name placeholder='End Date' />
                                    </Form.Item>
                ...
                <Table size='small'
                    ...
                    columns={columns}
                    loading={getEnquiryStatus === STATUS.FETCHING || getOfflineCourseStatus === STATUS.FETCHING}
                    pagination={{
            ...
            {toggleEditEnquiryModal && <EditEnquiry data={toggleEditEnquiryModal} closeModal={closeModal} />}
            {toggleRemarksDrawer && <EnquiryRemarkList data={toggleRemarksDrawer} closeDrawer={closeDrawer} />}
        </div>
    )
}
...
const EditEnquiry = ({ data, closeModal }) => {
    ...
    return (
        <Modal width={700} title='Update Enquiry' okText='Upload' visible={data ? true : false} footer={false} onCancel={closeModal}>
            <Card >
                <Form
                    form={form}
                    labelCol={{ span: 4 }}
                ...
                >
                    <Card style={{ padding: 0, border: 0 }}>
                        <Form.Item label="Status" initialValue={data?.status} rules={[{ required: true, message: 'Please pick a status.' }]} name='status'>
                            <Radio.Group required>
                                <Radio.Button required value="OPEN" >OPEN</Radio.Button>
                                <Radio.Button required value="CLOSED">CLOSED</Radio.Button>
                                {/* <Radio.Button required value="ADMITTED">ADMITTED</Radio.Button> */}
                            </Radio.Group>
                        </Form.Item>
                        <Form.Item label="Remark" rules={[{ required: true, message: 'Please fill in the field.' }]} name='remark'>
                            <Input.TextArea placeholder='Write remark here...' />
                        </Form.Item>
                    </Card>
                    <Form.Item wrapperCol={{ offset: 15 }}>
                        <Space>
                            <Button type="" span={4} onClick={closeModal}>
                                Cancel
                            </Button>
                            <Button span={4} type="primary"
                                htmlType="submit"
                            >
                                Update
                            </Button>

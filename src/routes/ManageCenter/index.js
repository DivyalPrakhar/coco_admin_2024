...
import { ExamCentersModal } from "../ManagePackages/ExamCentersModal";
  ...
  useEffect(() => {
    setFilterdData(
      filter(
        ...
        (f) =>
          (!input?.name ||
            f.name?.toLowerCase().includes(input?.name?.toLowerCase())) &&
          (!input?.code ||
            f.code?.toLowerCase().includes(parseInt(input?.code)))
      )
    );
  }, [allCenterList, input]);

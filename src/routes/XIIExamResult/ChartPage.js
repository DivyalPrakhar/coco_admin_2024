...
const { Option } = Select;
const SUBJECTS = [ 'HINDI', 'ENGLISH', 'BIOLOGY', 'CHEMISTRY', 'MATHEMATICS', 'PHYSICS' ]
...
export function ChartPage(props) {
    ...
    const subjectPerformanceAnalysisData = useMemo( () => { 
        ...
        if(data.subjectWisePercentGroupOfSchool){
                ...
                const schoolTotalStudent = _.sumBy(data.subjectWisePercentGroupOfSchool[sub], 'count');
                const subjectData = _.map( data.subjectWisePercentGroupOfSchool[sub], s => {
                    return ({
                        name: schoolName,
                        range: s._id + "%",
                ...
                return { subject: sub , data:subjectData };
            })
        }
        if(data.subjectWisePercentGroupOfDistrict){
            districtData = _.map(SUBJECTS, sub => {
            const districtTotalStudents = _.sumBy(data.subjectWisePercentGroupOfDistrict[sub], 'count');
            const districtSubjectData = _.map( data.subjectWisePercentGroupOfDistrict[sub], s => {
                    return ({
                        name: 'District Wise',
                        range: s._id + "%",
                        value: parseFloat(parseFloat((s.count / districtTotalStudents) * 100).toFixed(2))
            ...
            return { subject: sub ,data:districtSubjectData }
            })
        }
    ...
    const printPaper = () => {
        const url = window.location.origin + ROUTES.PRINT_XII_RESULT + "?" + `district=${district.toUpperCase()}&schoolId=${choosenSchool}`;
                        ...
                        <div>
                            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '100vh', pageBreakAfter:'always', fontFamily: 'serif', padding: '10px', border: '1px solid black' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#2c2e35', textAlign: 'center', lineHeight: '10px' }}>XIIth Result Analysis</div>
                            ...
                            <div style={{ minHeight: '100vh', pageBreakAfter: 'always', fontFamily: 'serif', padding: '10px', border: '1px solid black' }}>
                                <HeaderContainer2 text='Biology, Chemistry, Scince, Math, English, Hindi performance analysis(district wise)' />
                                    ...
                                    <Row justify='center' gutter={16}>
                                        {
                                            _.map(SUBJECTS, (sub, i) => {
                                                const subjectData = _.orderBy(_.flatMap(_.filter(subjectPerformanceAnalysisData, s => s.subject === sub ).map( s => s.data)), 'range');
                                                if(!data.subjectWisePercentGroupOfSchool || !data?.subjectWisePercentGroupOfSchool[sub]) return;
                                                return (
                                                    <Col key={sub} span={12} style={{ display: 'flex', justifyContent: 'center' }}>
                                                        <div style={{ marginBottom: i < 4 && '50px', width: '330px' }}>
                                                            <SingleSubjectResult subject={sub} data={subjectData}/>
                                                        </div>
                                                    </Col>
                                                )
                                            })

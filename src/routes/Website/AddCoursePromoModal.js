...
export const AddCoursePromoModal = ({visible, closeModal, currentPromo, app, web, update}) => {
    ...
    const handleAddPromo = () => {
        let order = websiteData.courseOffers?.length && _.maxBy(websiteData.courseOffers, 'order')?.order ? _.maxBy(websiteData.courseOffers, 'order')?.order + 1  : 1 
        let data = []
        let testOffer = {...appPromo, html:websitePromos, order:currentPromo ? currentPromo.order : order}
        let websiteOffers = websiteData.courseOffers || []

        if(currentPromo){
            let courseOffers = [...websiteOffers]
            let indx = _.findIndex(courseOffers,o => o._id === currentPromo._id)
            courseOffers[indx] = testOffer
            data = {courseOffers, instituteId}
        }
        else{
            data = {courseOffers:[...websiteOffers, {...appPromo, html:websitePromos, order}], instituteId}
        }

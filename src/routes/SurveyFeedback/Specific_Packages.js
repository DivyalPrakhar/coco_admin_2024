import { Button, Card, Col, Empty, Form, Image, Input, List, Popover, Row, Select, Table, Tag } from 'antd'
...
import { ControlOutlined, DeleteOutlined, EditOutlined, PictureOutlined, InsertRowBelowOutlined, CloseCircleOutlined, MessageOutlined, UserOutlined, RedoOutlined } from '@ant-design/icons'
...
export const SpecificPackages = ({ getData } ,props) => {
    ...
    const [finalFilters, setFinalFilters] = useState(() => {
        if (queries.get('name') || queries.get('published')) {
            return { name: queries.get('name'), published: queries.get('published') }
        }
    ...
    const sendMessage = (id) => {
        let pkg = packages.packagesList?.length ? _.find(packages.packagesList, p => p._id == id) : null
    ...
    const columns = [
        ...
        {
            title: <b>Package</b>, dataIndex: 'package', key: 2, width: 280,
            render: d => (<Text>{bilingualText(d)}</Text>)
        },
        {
            title: <b>Published</b>, width: 10, dataIndex: 'published', key: 5,
            render: d => (<Tag color={d == 1 ? 'green' : d == 0 ? 'red' : d == 2 ? 'orange' : 'red'}>{publishStatuses[d || 0]}</Tag>)
        },
        { title: <b>Created At</b>, dataIndex: 'createdAt', key: 'createdAt', width: 10 },
        { title: <b>Students</b>, dataIndex: 'studentCount', render: d => d || 0, key: 'studentCount', width: 20 },
    ]


    let dataSource = useMemo(() => {
        return packages.packagesList?.length ?
            _.orderBy(packages.packagesList, ['createdAt'], ['desc']).map((pkg) => ({
                key: pkg._id,
                package: pkg.name,
    ...
    useEffect(() => {
        let data = {}
        if (finalFilters)
            data = { ...finalFilters, exams: finalFilters.exams ? [finalFilters.exams] : [] }
    ...
    const submitData = () => {
        const data = selectedRowKeys.map((i) => dataSource.filter((d) => d.key === i))
        getData(data)
    ...
    return (
        <div>
            <Card loading={configData.defaultDataStatus == STATUS.FETCHING || products.allProductStatus == STATUS.FETCHING || course.getCoursesStatus == STATUS.FETCHING} id='specificpackages'>
                {configData.defaultDataStatus == STATUS.SUCCESS || products.allProductStatus == STATUS.SUCCESS || course.getCoursesStatus == STATUS.SUCCESS ?
                    <div>
                        ...
                        >
                            <Button type="primary" disabled={!hasSelected} loading={loading} onClick={submitData} >
                                Set
                            </Button>
                            <span
                                style={{
                        ...
                        <Table bordered loading={packages.getPackagesStatus === STATUS.FETCHING}
                            dataSource={dataSource} columns={columns} key={dataSource?.length} rowSelection={rowSelection} 
                        />
                    </div>
                    :
                    <Empty />
